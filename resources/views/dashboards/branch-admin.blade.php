<x-ui.card accent>
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
            <p class="text-xs font-semibold uppercase tracking-wide text-brand-700">Admin Cabang</p>
            <h1 class="mt-1 text-2xl font-semibold text-navy">Pusat kendali harian cabang</h1>
        ...
        <div class="text-left sm:text-right">
            <p class="text-xs font-semibold uppercase tracking-wide text-ink-soft">Hari Ini</p>
            <p class="text-sm font-medium text-navy">{{ format_date_id(now()) }}</p>
        </div>
    </div>
</x-ui.card>
...
<section aria-labelledby="daily-summary">
    <div class="mb-3 flex flex-wrap items-center justify-between gap-2">
        <h3 id="daily-summary" class="text-base font-semibold text-navy">Ringkasan Harian</h3>
        <p class="text-xs text-ink-soft">Operasional cabang secara ringkas</p>
    </div>
    ...
    <x-branch-dashboard.dashboard-section title="Papan Antrean Kerja" description="Pekerjaan harian diatur berdasarkan tahap workflow cabang.">
        <div class="grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
            @foreach ($branchQueues as $queue)
                <div class="rounded-lg border border-hairline bg-navy-50 p-3">
                    <div class="flex items-center justify-between gap-2">
                        <h4 class="text-sm font-semibold text-navy">{{ data_get($queue, 'title', 'Antrean') }}</h4>
                        <span class="rounded-full bg-surface px-2 py-0.5 text-xs font-medium text-ink-soft">{{ format_number_id(collect(data_get($queue, 'items', []))->count()) }}</span>
                    </div>
                    <div class="mt-3 space-y-3">
                        @forelse (data_get($queue, 'items', []) as $item)
                            <x-branch-dashboard.queue-card :item="$item" />
                        @empty
                            <div class="rounded-lg border border-dashed border-hairline bg-surface px-3 py-6 text-center">
                            <p class="text-sm text-ink-soft">{{ data_get($queue, 'empty', 'Belum ada item pekerjaan.') }}</p>
                            </div>
                        @endforelse
                    </div>
                </div>
