<x-settings-shell title="Clinics">
    <div class="bg-white shadow-sm sm:rounded-lg">
        <div class="p-6 space-y-4">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <form method="GET" action="{{ route('settings.clinics.index') }}" class="flex items-center gap-2">
                    <input type="text" name="search" value="{{ $search }}" placeholder="Search name, code, city"
                ...
                <a href="{{ route('settings.clinics.create') }}" class="inline-flex items-center rounded-md bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-500">+ Create Clinic</a>
            </div>
                            ...
                            <tr>
                                ...
                                <td class="px-3 py-2">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        @if ($clinic->is_active)
                                            <form method="POST" action="{{ route('settings.clinics.deactivate', $clinic) }}">@csrf @method('PATCH')<button class="text-amber-600 hover:text-amber-500">Deactivate</button></form>
                                        @else
                                            <form method="POST" action="{{ route('settings.clinics.activate', $clinic) }}">@csrf @method('PATCH')<button class="text-green-600 hover:text-green-500">Activate</button></form>
                                        @endif
                                        <form method="POST" action="{{ route('settings.clinics.destroy', $clinic) }}" onsubmit="return confirm('Delete this clinic?');">@csrf @method('DELETE')<button class="text-red-600 hover:text-red-500">Delete</button></form>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr><td colspan="6" class="px-3 py-6 text-center text-gray-400">No clinics found.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
