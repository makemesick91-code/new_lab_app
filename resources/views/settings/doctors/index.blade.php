        ...
        <div class="p-6 space-y-4">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <form method="GET" action="{{ route('settings.doctors.index') }}" class="flex flex-wrap items-center gap-2">
                    ...
                    <select name="branch_id" class="rounded-md border-gray-300 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                        <option value="">Semua cabang RME</option>
                        @foreach ($rmeBranches as $branch)
                            <option value="{{ $branch->id }}" @selected($branchId === $branch->id)>{{ $branch->code }} — {{ $branch->name }}</option>
                        @endforeach
                    </select>
                    ...
                    @if ($search || $branchId)<a href="{{ route('settings.doctors.index') }}" class="text-sm text-gray-500 hover:text-gray-700">Atur Ulang</a>@endif
                </form>
