<x-settings-shell title="Delivery Queue">
    <div class="space-y-6">
        <div class="bg-white shadow-sm sm:rounded-lg">
            <div class="p-6 space-y-4">
                <form method="GET" action="{{ route('deliveries.index') }}" class="flex flex-wrap items-center gap-2">
                    <input type="text" name="search" value="{{ $filters['search'] }}" placeholder="Order #, delivery #, clinic, doctor, patient"
        ...
        @canany(['create_delivery', 'manage_delivery'])
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                <h3 class="font-semibold text-gray-800">Ready to Prepare</h3>
                                ...
                                <tr>
                                    ...
                                    <td class="px-3 py-2">
                                        <form id="create-delivery-{{ $order->id }}" method="POST" action="{{ route('deliveries.store') }}" class="flex items-center gap-2">
                                            ...
                                            <input type="text" name="delivery_notes" placeholder="Notes" class="rounded-md border-gray-300 text-xs">
                                        </form>
                                    </td>
                                    <td class="px-3 py-2 text-right">
                                        <button form="create-delivery-{{ $order->id }}" class="text-indigo-600 hover:text-indigo-500">Create Delivery</button>
                                    </td>
                                </tr>
                            @empty
                                <tr><td colspan="6" class="px-3 py-6 text-center text-gray-400">No QC_PASSED orders waiting for delivery.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($deliveries as $delivery)
                            <tr>
                                ...
                                <td class="px-3 py-2"><span class="inline-flex rounded-full bg-indigo-50 px-2 py-0.5 text-xs font-medium text-indigo-700">{{ $delivery->status }}</span></td>
                                <td class="px-3 py-2 text-gray-600">{{ optional($delivery->labOrder?->due_date)->format('Y-m-d') ?? '-' }}</td>
                                <td class="px-3 py-2 text-right">
                                    <a href="{{ route('deliveries.show', $delivery) }}" class="text-indigo-600 hover:text-indigo-500">View Detail</a>
                                </td>
                            </tr>
                        @empty
                            <tr><td colspan="7" class="px-3 py-6 text-center text-gray-400">No delivery records.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
