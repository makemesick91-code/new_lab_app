<x-settings-shell title="Quality Control Report">
    <div class="bg-white shadow-sm sm:rounded-lg p-6 space-y-4">
        <div class="flex flex-wrap items-center justify-between gap-3">
            <form method="GET" action="{{ route('reports.qc') }}" class="flex flex-wrap items-end gap-2">
                ...
                <div><label class="block text-xs text-gray-500">QC Result</label>
                    <select name="qc_status" class="mt-1 rounded-md border-gray-300 text-sm"><option value="">All</option>
        ...
        <div class="flex flex-wrap gap-3 text-sm">
            ...
            @endforeach
            <span class="rounded-md bg-amber-50 px-3 py-1 text-amber-700">Remakes: <strong>{{ number_format($summary['remake_count']) }}</strong></span>
        </div>

        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                <thead><tr class="text-left text-gray-500">
                    <th class="px-3 py-2 font-medium">Order #</th><th class="px-3 py-2 font-medium">Clinic</th>
                    <th class="px-3 py-2 font-medium">Inspector</th><th class="px-3 py-2 font-medium">Result</th>
                    <th class="px-3 py-2 font-medium">Completed</th>
                </tr></thead>
                <tbody class="divide-y divide-gray-100">
                    ...
                    @empty
                        <tr><td colspan="5" class="px-3 py-6 text-center text-gray-400">No QC records found.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
