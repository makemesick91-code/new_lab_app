...
<x-inventory.dashboard-section title="Ringkasan Nilai Persediaan" description="Nilai persediaan berbasis ledger untuk cabang saat ini." density="compact">
    <div class="grid gap-4 sm:grid-cols-2">
        ...
        <div class="rounded-lg bg-brand-50 p-4">
            <p class="text-xs font-semibold uppercase tracking-wide text-brand-700">Total Nilai Persediaan</p>
            <p class="mt-2 text-lg font-semibold tabular-nums text-brand-800">{{ format_currency_id((float) data_get($summary, 'inventory_value', 0)) }}</p>
        </div>
    </div>
