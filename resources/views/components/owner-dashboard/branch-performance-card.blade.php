...
<article {{ $attributes->merge(['class' => 'rounded-lg border border-gray-200 bg-white p-4 shadow-sm']) }}>
    <div class="flex items-start justify-between gap-3">
        <div>
            <h4 class="text-sm font-semibold text-gray-900">{{ data_get($branch, 'name', 'Cabang') }}</h4>
            <p class="mt-1 text-xs text-gray-500">{{ data_get($branch, 'description', 'Ringkasan performa cabang') }}</p>
        </div>
    ...
    <dl class="mt-4 grid grid-cols-2 gap-3 text-sm">
        ...
        <div>
            ...
            <dd class="mt-1 font-semibold tabular-nums text-gray-900">{{ data_get($branch, 'completionTime', 'Belum ada data') }}</dd>
        </div>
        <div>
