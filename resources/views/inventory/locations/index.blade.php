        ...
        <div class="p-6 space-y-4">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <form method="GET" action="{{ route('inventory.locations.index') }}" class="flex flex-wrap items-center gap-2">
                    <input type="text" name="search" value="{{ $filters['search'] ?? '' }}" placeholder="Cari lokasi"
                    ...
                    <button class="rounded-md bg-gray-800 px-3 py-2 text-sm font-medium text-white hover:bg-gray-700">Cari</button>
                    <a href="{{ route('inventory.locations.index') }}" class="text-sm text-gray-500 hover:text-gray-700">Atur Ulang</a>
                </form>
                <a href="{{ route('inventory.locations.create') }}" class="rounded-md bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-500">Tambah Lokasi</a>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                            ...
                            <tr>
                                ...
                                <td class="px-3 py-2">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('inventory.locations.show', $location) }}" class="text-gray-600 hover:text-gray-900">Lihat</a>
                                        <a href="{{ route('inventory.locations.edit', $location) }}" class="text-indigo-600 hover:text-indigo-500">Ubah</a>
                                        @if ($location->is_active)
                                            <form method="POST" action="{{ route('inventory.locations.destroy', $location) }}">
                                                @csrf @method('DELETE')
                                                <button class="text-amber-600 hover:text-amber-500">Nonaktifkan</button>
                                            </form>
                                        @endif
                                    </div>
                                </td>
                        ...
                        @empty
                            <tr><td colspan="5" class="px-3 py-6 text-center text-gray-400">Belum ada lokasi persediaan.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
