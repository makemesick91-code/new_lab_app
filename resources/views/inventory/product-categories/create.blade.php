<x-settings-shell title="Tambah Kategori Produk">
    <div class="space-y-6">
        <div>
            <p class="text-xs font-semibold uppercase tracking-wide text-teal-700">Kategori Produk</p>
            <h2 class="mt-1 text-xl font-semibold text-gray-900">Tambah Kategori Produk</h2>
            <p class="mt-1 text-sm text-gray-500">Kategori membantu pengelompokan produk persediaan dalam cabang aktif.</p>
        </div>

        <section class="max-w-3xl rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
            <form method="POST" action="{{ route('inventory.product-categories.store') }}" class="space-y-5">
                @csrf
                ...
                <div class="flex items-center justify-end gap-2 border-t border-gray-200 pt-4">
                    ...
                    <button class="rounded-lg bg-teal-700 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-teal-600 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2">Simpan Kategori</button>
                </div>
            </form>
        </section>
    </div>
