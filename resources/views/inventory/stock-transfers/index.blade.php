                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($stockTransfers as $stockTransfer)
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-3">
                                    <p class="font-semibold text-gray-900">{{ $stockTransfer->transfer_number }}</p>
                                </td>
                                <td class="px-3 py-3 text-gray-700">{{ $stockTransfer->sourceInventoryLocation?->name ?? '-' }}</td>
                                <td class="px-3 py-3 text-gray-700">{{ $stockTransfer->destinationInventoryLocation?->name ?? '-' }}</td>
                                <td class="px-3 py-3 tabular-nums text-gray-600">{{ format_date_id($stockTransfer->transfer_date) }}</td>
                                <td class="px-3 py-3">
                                    @include('inventory.stock-transfers._status-badge', ['status' => $stockTransfer->status])
                                </td>
                                <td class="px-3 py-3 text-right tabular-nums text-gray-700">{{ format_number_id($stockTransfer->items_count ?? 0) }}</td>
                                <td class="px-4 py-3 text-right">
                                    <div class="flex flex-wrap items-center justify-end gap-2">
                                        <a href="{{ route('inventory.stock-transfers.show', $stockTransfer) }}" class="font-medium text-teal-700 hover:text-teal-600">Detail</a>
                            ...
                            <tr>
                                <td colspan="7" class="px-4 py-12">
                                    <div class="mx-auto max-w-sm text-center">
                                        ...
                                        @can('create', StockTransfer::class)
                                            <a href="{{ route('inventory.stock-transfers.create') }}" class="mt-4 inline-flex items-center rounded-lg bg-teal-700 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-teal-600">
                                                Buat Transfer Stok
                                            </a>
                                        @endcan
                                    </div>
                    ...
                    <article class="p-4">
                        ...
                        <div class="mt-4 grid grid-cols-2 gap-3 text-sm">
                            ...
                            <div class="rounded-lg bg-gray-50 p-3 ring-1 ring-gray-100">
                                ...
                                <p class="mt-1 font-semibold tabular-nums text-gray-900">{{ format_number_id($stockTransfer->items_count ?? 0) }}</p>
                            </div>
                        </div>
                        <div class="mt-4 flex flex-wrap gap-2">
                            <a href="{{ route('inventory.stock-transfers.show', $stockTransfer) }}" class="rounded-lg border border-teal-200 px-3 py-2 text-sm font-medium text-teal-700">Lihat detail</a>
                            @can('update', $stockTransfer)
                                @if ($stockTransfer->status === StockTransfer::STATUS_DRAFT)
