...
<x-settings-shell title="Stok Opname {{ $stockOpname->opname_number }}">
    <div class="space-y-6">
        <div class="flex flex-wrap items-start justify-between gap-3">
            <div>
                <p class="text-xs font-semibold uppercase tracking-wide text-brand-700">Detail Stok Opname</p>
            ...
            <div class="flex flex-wrap items-center gap-2">
                <a href="{{ route('inventory.stock-opnames.review-screen', $stockOpname) }}" class="inline-flex items-center rounded-lg bg-brand-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2">
                    Tinjau Selisih
                </a>
                ...
                @endif
                <a href="{{ route('inventory.stock-opnames.index') }}" class="inline-flex items-center rounded-lg border border-gray-200 px-4 py-2 text-sm font-semibold text-gray-600 hover:bg-gray-50 hover:text-gray-900 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2">
                    Kembali
                </a>
            </div>
        </div>
            ...
            <div class="lg:col-span-2">
                <div class="rounded-lg border border-gray-200 bg-white shadow-sm">
                    <div class="flex flex-wrap items-center justify-between gap-3 border-b border-gray-200 px-4 py-3">
                        ...
                        @if (in_array($stockOpname->status, ['DRAFT', 'COUNTING']))
                            <button type="button" class="inline-flex items-center rounded-lg bg-brand-700 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-brand-600 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2" onclick="document.getElementById('add-product-form').classList.toggle('hidden')">
                                Tambah Produk
                            </button>
                        @endif
                    </div>
                                            ...
                                            @if (in_array($stockOpname->status, ['DRAFT', 'COUNTING']))
                                                <form method="POST" action="{{ route('inventory.stock-opnames.update-counted-quantity', [$stockOpname, $item->product_id]) }}" class="flex items-center justify-end gap-2">
                                                    ...
                                                    <input type="number" step="0.0001" min="0" name="counted_quantity" value="{{ (string) ($item->counted_quantity ?? 0) }}"
                                                           class="w-32 rounded-lg border-gray-300 text-sm text-right focus:border-brand-500 focus:ring-brand-500">
