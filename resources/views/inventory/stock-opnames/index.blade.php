<x-settings-shell title="Stock Opnames">
    <div class="space-y-6">
        <div class="flex flex-wrap items-start justify-between gap-3">
            <div>
                <p class="text-xs font-semibold uppercase tracking-wide text-teal-700">Inventory Stock Opnames</p>
                <h2 class="mt-1 text-xl font-semibold text-gray-900">Stock Opname Directory</h2>
                <p class="mt-1 text-sm text-gray-500">Physical stock count sessions and variance adjustments.</p>
            </div>
        ...
        <form method="GET" action="{{ route('inventory.stock-opnames.index') }}" class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
            <div class="grid gap-3 md:grid-cols-[minmax(0,1fr)_12rem_12rem_12rem_auto_auto] md:items-end">
                <div>
                    <label for="stock-opname-search" class="text-sm font-medium text-gray-700">Search stock opname</label>
                    <input id="stock-opname-search" type="search" name="search" value="{{ $filters['search'] ?? '' }}" placeholder="Number or location"
                           class="mt-1 block w-full rounded-lg border-gray-300 text-sm focus:border-teal-500 focus:ring-teal-500">
                </div>
                <div>
                    <label for="stock-opname-location" class="text-sm font-medium text-gray-700">Location</label>
                ...
                <div>
                    <label for="stock-opname-date-from" class="text-sm font-medium text-gray-700">From date</label>
        ...
        <section class="rounded-lg border border-gray-200 bg-white shadow-sm">
            <div class="flex flex-wrap items-center justify-between gap-3 border-b border-gray-200 px-4 py-3">
                <div>
                    <h3 class="text-base font-semibold text-gray-900">Stock Opnames</h3>
                    <p class="text-sm text-gray-500">{{ number_format($stockOpnames->total()) }} stock opnames in the active branch scope.</p>
                </div>
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-200 text-sm">
                    <thead class="bg-gray-50">
                        <tr class="text-left text-gray-500">
                            <th scope="col" class="px-4 py-3 font-medium">Opname Number</th>
                            ...
                            <th scope="col" class="px-3 py-3 font-medium">Counted By</th>
                            <th scope="col" class="px-3 py-3 font-medium">Items</th>
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex flex-wrap items-center justify-end gap-2">
                                        <a href="{{ route('inventory.stock-opnames.show', $stockOpname) }}" class="font-medium text-teal-700 hover:text-teal-600">View</a>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="7" class="px-4 py-12">
                                    <div class="mx-auto max-w-sm text-center">
                                        ...
                                        <p class="mt-3 text-sm font-medium text-gray-900">No stock opnames found.</p>
                                        <p class="mt-1 text-sm text-gray-500">Create a stock opname to start counting physical inventory.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
                    ...
                    <article class="p-4">
                        ...
                        <div class="mt-4 grid grid-cols-2 gap-3 text-sm">
                            <div class="rounded-lg bg-white p-3 ring-1 ring-gray-100">
                                <p class="text-xs text-gray-500">Counted By</p>
                        ...
                        <div class="mt-4 flex flex-wrap gap-2">
                            <a href="{{ route('inventory.stock-opnames.show', $stockOpname) }}" class="rounded-lg border border-teal-200 px-3 py-2 text-sm font-medium text-teal-700">View</a>
                        </div>
                    </article>
                @empty
                    <div class="px-4 py-10 text-center">
                        <p class="text-sm font-medium text-gray-900">No stock opnames found.</p>
                        <p class="mt-1 text-sm text-gray-500">Create a stock opname to start counting physical inventory.</p>
                    </div>
                @endforelse
            </div>
