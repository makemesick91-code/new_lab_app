        ...
        <x-ui.card>
            <form method="POST" action="{{ route('inventory.products.update', $product) }}" class="space-y-5">
                ...
                <div class="flex items-center justify-end gap-2 border-t border-gray-100 pt-5">
                    <x-ui.button variant="secondary" :href="route('inventory.products.show', $product)">Batal</x-ui.button>
                    <x-ui.button type="submit" variant="primary">Perbarui Produk</x-ui.button>
                </div>
            </form>
        </x-ui.card>
    </div>
