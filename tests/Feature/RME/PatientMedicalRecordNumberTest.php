...
it('composes the finalized DG format', function () {
    expect($this->service->compose('TLK1', 2026, '0001'))
...
it('derives the year from the registration date', function () {
    expect($this->service->composeForRegistration('TLK1', Carbon::parse('2026-03-15'), '0001'))
        ->toBe('DG-TLK1-2026-0001');
});

it('rejects a non four digit year', function () {
    expect(fn () => $this->service->compose('TLK1', 26, '0001'))
...
it('requires a branch code and a manual number', function () {
    expect(fn () => $this->service->compose('', 2026, '0001'))->toThrow(InvalidArgumentException::class)
        ->and(fn () => $this->service->compose('TLK1', 2026, '  '))->toThrow(InvalidArgumentException::class);
});
...
it('detects an existing final medical record number', function () {
    ...
    expect($this->service->exists('DG-TLK1-2026-0001'))->toBeTrue()
        ->and($this->service->exists('DG-TLK1-2026-9999'))->toBeFalse();
});
...
it('does not auto-generate or auto-increment the manual number', function () {
    ...
    $first = $this->service->compose('TLK1', 2026, '0001');
    $second = $this->service->compose('TLK1', 2026, '0001');

    expect($first)->toBe($second)->toBe('DG-TLK1-2026-0001');
});

it('allows the same manual number across branches because the final value differs', function () {
    $a = $this->service->compose('TLK1', 2026, '0001');
    $b = $this->service->compose('LDK2', 2026, '0001');

    expect($a)->not->toBe($b)
        ->and($a)->toBe('DG-TLK1-2026-0001')
