...
it('provisions no production key, arms no certificate, and moves no gate', function () {
    // Phase 4A authorization changes nothing about signing, and that remains
    // the claim. The key that now exists was generated by
    // PRODUCTION-ANDROID-SIGNING-KEY-PROVISIONING-1 under its own authority,
    // and the pin was armed by PRODUCTION-ANDROID-SIGNING-CERTIFICATE-PIN-1
    // under its own — neither by this readiness work.
    //
    // So the assertion is no longer "nothing is pinned", which stopped being
    // this suite's invariant the moment a separate task pinned something. It
    // is that the pilot authorization did not introduce an anchor of its own:
    // whatever is in force is the certificate custody recorded.
    ...
    // The gates this suite actually owns. Pinning a trust anchor is upstream
    // of all three and implies none of them.
