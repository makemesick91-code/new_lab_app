...
/**
 * REVISION-TELKOMAS-BRANCH-CODE-TKM1-TO-TLK1-1 — convert Cabang Telkomas'
 * deprecated branch code to its canonical one, in the places where the value is
 * ACTIVE CANONICAL DATA and nowhere else.
 *
 ...
 * column", and there is deliberately no `replace(col, 'TKM1', 'TLK1')` anywhere:
 * a blind replacement would also rewrite the string inside a manual RM sequence,
 * inside an approval reference, inside a JSON payload, and inside a patient's
 * free text — changing meaning it was never asked to touch. Nomor RM values go
 * through the canonical parser/composer instead, so only the branch-code SEGMENT
 * can ever change and a value that is not a Nomor RM is left alone.
 *
 ...
 *                                    TKM1. Editing it would make the record
 ...
 *                                    source Nomor RM transcribed from the paper
 *                                    document. The document really does say
 *                                    TKM1; the archive's job is to say so.
 ...
 * second run is a no-op. That matters because a deployment may already have been
 * migrated by hand before this shipped — which is exactly the state production
 * was found in.
 */
return new class extends Migration
{
     ...
     *
     * A down() would have to turn `TLK1` back into `TKM1`, and after this
     * migration runs the application ISSUES canonical codes: new patients, new
     * visits and new rollout state legitimately carry TLK1. Nothing in the data
     * distinguishes "TLK1 because it was migrated" from "TLK1 because it was
    ...
    private function assertNoBranchCodeCollision(): void
    {
        ...
        $historical = BranchCodeAlias::TELKOMAS_HISTORICAL;
        $canonical = BranchCodeAlias::TELKOMAS_CANONICAL;
        ...
        if ($canonicalRows->isNotEmpty()) {
            throw new RuntimeException(sprintf(
                'REVISION-TELKOMAS-BRANCH-CODE: branch code %s is already held by branch id(s) %s while branch id(s) %s still hold %s. '
        ...
        if ($historicalRows->count() > 1) {
            throw new RuntimeException(sprintf(
                'REVISION-TELKOMAS-BRANCH-CODE: branch code %s is held by more than one branch (ids %s). Resolve the duplicate before migrating.',
                $historical,
                $historicalRows->implode(', '),
            ));
        }
    ...
    private function renameBranchCode(): void
    {
        ...
        DB::table('mst_branches')
            ->where('code', BranchCodeAlias::TELKOMAS_HISTORICAL)
            ->update(['code' => BranchCodeAlias::TELKOMAS_CANONICAL]);
    }
    ...
    private function migratePatientMedicalRecordNumbers(): void
    {
        ...
        $candidates = DB::table('mst_patients')
            ...
            ->where('medical_record_number', 'like', 'DG-'.BranchCodeAlias::TELKOMAS_HISTORICAL.'-%')
            ->orderBy('id')
            ->lockForUpdate()
            ->get();
        ...
        foreach ($candidates as $candidate) {
            ...
            if ($collidesWith->isNotEmpty()) {
                throw new RuntimeException(sprintf(
                    'REVISION-TELKOMAS-BRANCH-CODE: patient id %d cannot be migrated because its canonical Nomor RM is already held by patient id(s) %s. '
     ...
     * admission to its own wave. Terminal waves are skipped: they authorize
     * nothing, and their record of what was approved stays as it was written.
     */
