...
**Sprint 22 Phase 22.0 — Planning & Baseline (2026-06-11):** Branch `feature/sprint-22-planning`. Tag `sprint-22-planning`. Planning doc: `docs/sprint_22_planning.md`. Current focus: pilot stabilization, role/permission/menu hardening for RME/cashier/lab candidates/owner/admin users, RME end-to-end smoke validation, cashier/payment/receipt stabilization, RME -> LabCaseCandidate validation, and read-only Owner Dashboard foundation. Documentation only in Phase 22.0. Preserve Sprint 21 constraints: no `migrate:fresh`/`db:wipe` on VPS, no RME payment behavior changes, no auto LabOrder from RME payment, keep `LabCaseCandidate` staging, no lab payment records from RME payment, SOAP doctor UI hidden, handwriting-first RM, and Owner Dashboard read-only in Sprint 22.
