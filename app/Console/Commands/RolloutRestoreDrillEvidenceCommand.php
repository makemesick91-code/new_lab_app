...
class RolloutRestoreDrillEvidenceCommand extends Command
{
    ...
    private function renderConsole(array $result): void
    {
        ...
        if (! empty($details)) {
            $this->newLine();
            $this->line('  Details:');
            foreach (['evidence_present', 'evidence_file', 'environment', 'restore_target', 'production_overwrite', 'source_backup_file', 'source_backup_size_bytes', 'age_hours', 'stale', 'evidence_decision'] as $k) {
